# %%
"""
Interactive Data Loading and Exploration Tool for Churn Analysis
...
to load, process, and analyze customer data from Snowflake with real-time
...
logger = get_logger()
logger.info("=" * 80)
logger.info("Starting Manual Data Loading Example")
logger.info("=" * 80)
# %%
...
logger.info("=" * 80)
    ...
    if not training_data.empty:
        ...
        # Get and display monthly churn distribution early
        print("\n" + "=" * 80)
        print("NUMBER OF CHURNS PER MONTH DISTRIBUTION")
        print("=" * 80)
        
        churn_dist_early = data_loader.get_monthly_churn_distribution()
        if churn_dist_early:
            dist_stats_early = churn_dist_early['distribution_stats']
            print("\nMonthly churn count statistics:")
            print(f"   Mean: {dist_stats_early['mean_churns_per_month']:.1f} churns/month")
            print(f"   Median: {dist_stats_early['median_churns_per_month']:.1f} churns/month") 
            print(f"   Min: {dist_stats_early['min_churns_per_month']:.1f} churns/month")
            print(f"   Max: {dist_stats_early['max_churns_per_month']:.1f} churns/month")
            print(f"   Std Dev: {dist_stats_early['std_churns_per_month']:.1f} churns/month")
            
            print(f"\nTotal months analyzed: {dist_stats_early['total_months']}")
            print(f"Total churned customers: {dist_stats_early['total_churned_customers']}")
            print(f"Average monthly churn rate: {(dist_stats_early['mean_churns_per_month'] / dist_stats_early['total_churned_customers'] * 100):.2f}% of all churned")
